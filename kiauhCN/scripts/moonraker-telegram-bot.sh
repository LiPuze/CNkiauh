...
function clone_telegram_bot() {
  local repo=${1}
  unzip "${HOME}/kiauh_local/moonraker-telegram-bot-master.zip"  -d "${HOME}/kiauh_local"
  ...
  cd "${HOME}" || exit 1
  if ! cp -RT "${HOME}/kiauh_local/moonraker-telegram-bot-master" "${TELEGRAM_BOT_DIR}"; then
