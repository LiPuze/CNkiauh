...
#          printf "|${cyan}%-57s${white}|\n" " ● moonraker-obco-$(get_instance_name "${svc}" moonraker-obico)"
  ...
  if (( ${#not_linked_instances[@]} > 0 )); then
    ...
    echo -e "| 1. 'cd ${HOME}/kiauh && ./kiauh.sh' to launch KIAUH.        |"
...
function clone_moonraker_obico() {
  local repo=${1}
  unzip "${HOME}/kiauh_local/moonraker-obico-master.zip"  -d "${HOME}/kiauh_local"
  ...
  cd "${HOME}" || exit 1
  if ! cp -RT "./kiauh_local/moonraker-obico-master" "${MOONRAKER_OBICO_DIR}"; then
